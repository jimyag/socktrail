...
`SYN RTT` 只对看到本机发出 SYN、收到匹配 SYN-ACK 的 TCP 连接给出一次握手时差样本；入站或中途开始时为 `-`。
...
两列是网络诊断提示，不与报文字节另行相加。DNS 流另有应答时延，见[解析原理](parsing.md#dns-与-ssh)；它和 `SYN RTT` 一样是采集点之后的往返，本机出口排队的时间不在其中。
