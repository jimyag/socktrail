...
进度（2026-09-25）：0、2、9、10、16 已实现并验证；1 已实现，长时间 RSS 验证进行中；3A 的 Docker bridge/host/veth 行为已实测，3B 未开始；4 的 Docker 名称和 `--container` 已在实机验证，Pod 名称有本地文件测试但未在真实 kubelet 主机验收；17 的协议升级 TLS、PROXY v2 AUTHORITY、按进程 DNS 关联和加密 DNS 标记已实现并验证。其余条目尚未开始。
