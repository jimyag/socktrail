...
已实现 `--read`：复用现有报文解析，补充录制时的进程和域名注释；终端显示最终静态界面，非终端输出文本报告，`--output json` 输出快照。无探针、无 `/proc` 读取；不模拟录制时间轴。读写往返测试覆盖接口、时间、进程和域名，损坏块会报错。逐秒回放只有真实复盘需求时再增加。
