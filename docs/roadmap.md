...
从 [文档索引](README.md) 进入。这里列出尚未实现的功能，每项写明目标、现状、做法和验收方式，供实现者（人或 AI）直接动手。状态截至 2026-09-25。开始任何一项之前，先读完[通用约定](#通用约定)。
...
验收：
- fieldalignment 对 `flow` 不再报告浪费。
- `BenchmarkFlowMemory` 前后对比，B/flow 下降约 128。
- 新增一个测试，断言 `unsafe.Sizeof(flow{}) <= 768`，防止以后再越过这个规格。
...
- 场景和预期：
  1. bridge 容器访问外网：网桥口和物理口都能看到这条连接，conntrack 把 NAT 前后合并成一条，方向为 `forwarded`，没有 PID。
  2. host 网络容器：进程完整，服务显示为 `docker-<id>.scope`。
  3. 同一网桥上的两个容器互访：只在网桥上可见，没有 PID。
