...
5. **交付验证：进行中**。README 与验证记录已补齐；2,500 次短连接压力样本和本机 `sendfile` 对照通过，仍需极限吞吐、跨内核、NAT、splice/内核 TLS 等路径及数小时运行验收。退出后应不留探针或后台进程。容器、域名拦截和 GeoIP 不在本轮范围。

## 依赖判断

报文采集选 Linux AF_PACKET；PID 探针选 cilium/ebpf 的内嵌 CO-RE 对象，不需要运行时 bpftrace。socket 事件端点与所选接口报文五元组能在已测本机直连场景对应，`fork` 后子进程读写共享 socket 已实测，NAT、fd 传递、多写者和短连接极端时序仍是风险。若目标机不支持所需 fexit/BTF，不应静默降级为无 PID 统计。
