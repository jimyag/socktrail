...
| 服务 | 进程所在的 systemd 服务或容器（cgroup 路径里最内层的 `.service` 或 `.scope`，容器 ID 缩到 12 位）；`g` 切换为完整 cgroup 路径或进程树（一个进程和它在同一服务里的子孙进程）。收发量是组内进程的 socket 字节之和 | 组内进程参与的连接，第一列 `I/O PID` 是组内在这条连接上有收发的进程，父进程接受连接后交给子进程收发时，这里是子进程；`process` 标签按父子关系缩进列出组内进程 |
...
- `/` 过滤 IP、域名、PID 或进程名，输入时 `q` 是普通字符，`Enter` 确认、`Esc` 清除；`s` 恢复主表总字节/总速率排序并切换两者；`0` 打开或关闭网卡诊断总览，`i` 切换接口，`a` 返回整机视图；`?` 查看帮助，`!` 查看采集状态，`q` 退出，`Ctrl-C` 在任何状态下都退出。
