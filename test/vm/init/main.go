...
func main() {
	defer syscall.Reboot(syscall.LINUX_REBOOT_CMD_POWER_OFF)
...
func (r *result) capture() {
	...
	for lines.Scan() {
		...
		fmt.Sscanf(line, "OpenSSL SNI events %d", &r.opensslEvents)
		if rest, ok := strings.CutPrefix(line, "capture status: AF_PACKET delivered="); ok {
			var delivered int
			fmt.Sscanf(rest, "%d dropped=%d", &delivered, &r.dropped)
		}
	}
	err := cmd.Wait()
...
func traffic() {
	...
	if text, err := os.ReadFile("/data/quic-initial.hex"); err == nil {
		...
		if err == nil {
			...
			quic.WriteToUDP(initial, &net.UDPAddr{IP: net.IPv4(127, 0, 0, 1), Port: 443})
			quic.Close()
		}
	}
	...
	if raw, err := net.ListenPacket("ip4:icmp", "127.0.0.1"); err == nil {
		for seq := range byte(3) {
			raw.WriteTo(echo(0x4242, seq), &net.IPAddr{IP: net.IPv4(127, 0, 0, 1)})
		}
		raw.Close()
	}
